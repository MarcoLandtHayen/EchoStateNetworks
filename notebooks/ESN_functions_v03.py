...
class ESN(tf.keras.layers.Layer):
    def __init__(self, n_res, W_in_lim=1, leak_rate=0.5, activation='tanh', verbose=0):
    ...
    def call(self, inputs):
        ...
        if self.activation=='tanh':
            ...
            ### Question: Do I consider leak_rate for initial reservoir states?
            ## If yes:
            x_prev = self.leak_rate * tf.keras.activations.sigmoid(self.res_units_init(inputs[:,0:1,:])) # x(1) = leak_rate * tanh(W_in * u(1))
            ## If no:
            #x_prev = tf.tanh(self.res_units_init(inputs[:,0:1,:])) # x(1) = tanh(W_in * u(1))
...
def setESN(input_length, n_features, n_layers, n_res, W_in_lim, leak_rate, activation, spec_radius, sparsity, verbose=False):
    ...
    for l in range(n_layers):
        ...
        if l == 0:
            ...
            X, X_T = ESN(n_res=n_res, W_in_lim=W_in_lim, leak_rate=leak_rate, activation=activation)(model_inputs)
        ...
        # Further ESN layers need to be connected to previous ESN layer:
            ...
            X, X_T = ESN(n_res=n_res, W_in_lim=W_in_lim, leak_rate=leak_rate, activation=activation)(X)
